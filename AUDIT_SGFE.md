...
> **Mise à jour du 5 septembre 2026 (cette PR)** : **Journalisation de sécurité centralisée et inviolable** (§J) passe de 🟡 partiel à ✅ fait — nouveau RPC `EnregistrerEvenementSecurite` qui centralise les événements de sécurité de la gateway dans l'`AuditLog` du service Auth (best-effort, non bloquant), et nouveau mécanisme de chaînage de hash tamper-evident (`libs/sgfe_common/sgfe_common/log_integrity.py`), câblé sur Auth et Gateway — voir le détail complet dans l'item §J ci-dessus. Un seul item change de colonne (P1 : partiel→fait), portant le total à **81 (84 %)** faits, 2 partiels, sur les mêmes 97 items — recompté avec `scripts/compter_audit.py`. Nouveau document `docs/PREUVES_SOC2.md`, qui décline formellement l'item "Démarrer la collecte de preuves SOC 2" (reste 🟡, par nature continu). **Aucun autre item n'a changé** : l'observabilité (§I) reste le seul chantier à zéro, et reste le seul point qui ferait échouer un audit SOC 2 Type II aujourd'hui.
