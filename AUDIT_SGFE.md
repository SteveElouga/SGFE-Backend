...
> **Mise à jour — 05/09/2026 : l'immuabilité niveau base est désormais RÉELLE pour paiement et facturation** (nouvelles migrations `paiements/0015_audit_log_role_runtime` et `factures/0010_audit_log_role_runtime`). L'investigation a montré que la limite ci-dessus était en réalité plus grave que documenté : le rôle `POSTGRES_USER` de l'image Postgres officielle est un **superutilisateur** (attribut hérité du bootstrap `initdb`), qui contourne TOUT contrôle d'accès — pas seulement la propriété d'une table — vérifié empiriquement sur un conteneur Postgres jetable (`ALTER TABLE ... OWNER TO <tiers>` + `REVOKE ALL` ne bloque pas davantage un superutilisateur qu'un simple `REVOKE`). Mécanisme retenu, **sans toucher à `docker-compose.yml` ni introduire de nouveau secret** : chaque service crée (via migration `RunPython`, sous le rôle propriétaire) un second rôle Postgres `NOLOGIN` non superutilisateur (`<rôle>_runtime`), avec `SELECT, INSERT` uniquement sur `audit_log` et tous les droits sur le reste du schéma (existant + futur, via `ALTER DEFAULT PRIVILEGES`) ; la connexion applicative (`grpc_server`, jamais `migrate`) y bascule via `SET ROLE` dès l'établissement de chaque connexion (`<app>/db_hardening.py`, câblé sur le signal Django `connection_created`) — `SET ROLE` change l'identité effective de la session sans changer sa connexion réseau, donc `enregistrer_audit` reste dans la même transaction que la mutation métier qu'il documente (aucune régression d'atomicité). **Vérifié réellement, pas seulement en théorie** : suite de tests d'intégration contre un Postgres jetable (`test_db_hardening_postgres.py`, gatée par `FORCE_POSTGRES_TESTS`, qui tourne déjà dans le job CI `test-paiement`/`test-facturation`) prouvant qu'après `SET ROLE`, `UPDATE`/`DELETE` échouent avec `permission denied` et que la session perd réellement son statut superutilisateur (`rolsuper = false`), là où le même rôle réussissait toujours `UPDATE`/`DELETE` avant. Mécanisme conçu comme **générique** dès le départ (module `libs/sgfe_common/sgfe_common/db_hardening.py`, synchronisé par `scripts/sync-db-hardening-lib.sh`, vérifié en CI comme `sgfe_common.grpc_auth`) pour que campagne/abonné/auth/config le répliquent pour leur propre `AuditLog` sans réinventer l'analyse — voir le commentaire de tête de `db_hardening.py` pour la marche à suivre en 4 étapes. Ce qui reste, inchangé : campagne/abonné/auth/config n'ont toujours pas leur `AuditLog` (item toujours 🟡 partiel pour cette raison, pas pour l'immuabilité base qui est close pour les 2 services existants).
