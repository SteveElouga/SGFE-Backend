...
- [x] **RGPD** : export + effacement/anonymisation. *(M)* — **✅ Fait — PR #179 (abonné, 03/09) puis étendu PR #213 (utilisateurs internes, 05/09).** Abonné : `ExporterDonneesAbonne` (droit à la portabilité, dégradation gracieuse par section) et `AnonymiserAbonne` (droit à l'effacement, uniquement `RESILIE`, jamais factures/paiements). Auth (05/09) : même esprit pour les comptes internes (ADMIN/AGENT/COMPTABLE/SUPERVISEUR) — `ExporterDonneesUtilisateur` et `AnonymiserUtilisateur` (refuse si le compte est actif ; anonymise `username`/`email`/`phone_number`, dérivés de l'UUID car uniques en base, contrairement à l'Abonné où une valeur littérale suffisait). Mutations gateway ADMIN uniquement des deux côtés. **Non couvert** : complétude juridique des champs exportés, non validée par un juriste — mécanisme technique seulement. **Mise à jour (09/09/2026) — périmètre étendu et tranché noir sur blanc sur les 7 services restants** : voir `docs/RGPD_PERIMETRE_EFFACEMENT.md`. Un seul écart réel trouvé — `notification` stockait le téléphone (chiffré par sa propre clé Fernet, donc toujours déchiffrable après anonymisation côté Abonné) et le dernier message WhatsApp (texte en clair embarquant le prénom/nom) de chaque envoi ; `EnvoiService.anonymiser_envois_abonne`/`DiffusionService.anonymiser_envois_abonne` corrigent ce point, avec cascade depuis la mutation gateway `anonymiserAbonne`. Les 6 autres (`campagne`, `facturation`, `paiement`, `reporting`, `config`, `gateway`) sont déjà conformes par conception — aucune PII d'abonné stockée en propre (vérifié champ par champ, pas supposé), ou pas de base de données propre pour `gateway`.
...
>
> **Ce qui reste, 4 septembre** : piste d'audit (§J) entamée mais **incomplète** — `AuditLog` manque encore sur campagne/abonné/auth/config (une PR par service, dans cet ordre, prévu par la conception §10.7), la journalisation de sécurité ne couvre que la gateway, et le REVOKE niveau base a une limite documentée (rôle applicatif = propriétaire de la table dans la configuration actuelle). **Mise à jour du 5 septembre : `AuditLog` étendu le jour même aux 4 services restants** (PR #205/#208/#210/#212, voir §J et le tableau Go/No‑Go ci-dessus) — les 6 services prévus par la conception §10.7 sont désormais couverts, plus aucun point non fait sur cet item. L'immuabilité base (rôle Postgres non superutilisateur) a suivi le même périmètre le lendemain (voir le complément du 06/09 en tête de document). **Observabilité (§I)** reste le seul chantier à zéro, jamais entamé. Tarification par tranches, pénalités de retard et estimation automatique des compteurs — **déclinés explicitement par l'utilisateur** le 3 septembre (staging iso-prod, CGU et multi-tenant classés « pas d'actualité », pas refusés). **Ce qui reste, P2** : uniquement l'environnement de staging iso-prod (non fait, explicitement pas d'actualité pour l'instant).
>
...
>
> **Mise à jour — 5 septembre 2026 : étape 2 étendue aux 4 services restants, dans l'ordre prévu ci-dessus** — config (PR #205), campagne (PR #208, fusionné avec `ReleveAudit`), auth (PR #210), abonné (PR #212). Les **6 services concernés par la conception ont désormais `AuditLog`**. Étape 3 (immuabilité base, REVOKE réel via rôle Postgres non superutilisateur) suit le même périmètre le lendemain : livrée pour paiement/facturation le 05/09 (PR #215), puis **étendue à campagne/config/auth/abonné le 06/09/2026** (voir le complément de fraîcheur en tête de document) — **6/6 services isolés**, plus aucun `REVOKE` symbolique sans effet. Étape 4 reste non commencée, comme prévu.
>
> **Mise à jour — 9 septembre 2026 : l'exclusion de Notification, assumée ci-dessus et en §8·J, ne tenait qu'à moitié pour 3 RPC précises.** Le principe reste (Notification exécute des envois déclenchés par un événement amont déjà tracé côté service d'origine — pas une nouvelle mutation d'état) mais 3 RPC en étaient une exception non traitée : `CreerDiffusion` (lance une campagne de message libre vers potentiellement des centaines d'abonnés), `RevoquerToken` et `RevoquerTousTokens` (retirent l'accès à l'espace abonné, un par un ou en masse) — trois vraies mutations d'état sensible, pas de simples envois. Corrigé : `notifications/audit.py` + `AuditLog` (migrations `0009_auditlog`/`0010_audit_log_immutable`/`0011_audit_log_role_runtime`, même mécanisme exact que les 6 autres services, y compris l'isolation Postgres `_runtime` non superutilisateur dès l'origine — `libs/sgfe_common/sgfe_common/db_hardening.py` ajouté à `scripts/sync-db-hardening-lib.sh`) câblés sur ces 3 RPC seulement (actions `DIFFUSION_CREEE`/`TOKEN_REVOQUE`/`TOUS_TOKENS_REVOQUES`), avec des détails strictement non-PII (comptes, identifiants techniques — jamais un numéro de téléphone ni le contenu d'un message). **7 services ont désormais `AuditLog`** ; Reporting reste seul hors périmètre par construction (agrégateur strictement read-only).
