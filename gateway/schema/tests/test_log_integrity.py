...
`schema/log_integrity.py` est une copie synchronisée de
...
(`gateway/settings.py`) pointe bien vers elle, sur le bon handler.
"""
...
from schema.log_integrity import GENESIS_HASH, LOG_HASH_SUFFIX_RE, ChainedHashFormatter
...
class LoggingSettingsWiringTests(SimpleTestCase):
    ...
    Couvre notamment le logger `security` (`schema/context.py`), qui écrit
    sur "file" comme tous les autres loggers de ce composant."""

    def test_formatter_chaine_pointe_vers_schema_log_integrity(self) -> None:
        ...
        self.assertEqual(formatter_chaine["()"], "schema.log_integrity.ChainedHashFormatter")
    ...
    def test_handler_console_n_utilise_pas_le_formatter_chaine(self) -> None:
        ...
        # tant que `TESTING=True` (voir `gateway/settings.py` : ajouté
