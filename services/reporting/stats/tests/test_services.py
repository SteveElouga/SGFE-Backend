...
class UpdateStatsCampagneTests(TestCase):
    ...
    def test_upsert_calcule_attente_et_pourcentage(self) -> None:
    ...
    def test_upsert_idempotent_ecrase_les_valeurs(self) -> None:
    ...
    def test_total_zero_pourcentage_zero(self) -> None:
...
class UpdateStatsFacturationTests(TestCase):
    ...
    def test_generee_incremente_total_montant_et_impayees(self) -> None:
    ...
    def test_payee_bascule_impayee_vers_payee(self) -> None:
    ...
    def test_annulee_d_une_facture_deja_payee_decremente_payees(self) -> None:
        self.agg.update_stats_facturation(self.cid, 10, 100000, "GENEREE")
    ...
    def test_annulee_ne_touche_pas_les_factures_envoyees(self) -> None:
    ...
    def test_annulee_ne_descend_jamais_sous_zero(self) -> None:
...
class UpdateStatsPaiementsTests(TestCase):
    ...
    def test_paiement_calcule_taux_et_impaye(self) -> None:
        self.agg.update_stats_facturation(self.cid, 10, 100000, "GENEREE")
    ...
    def test_paiement_cumule(self) -> None:
        self.agg.update_stats_facturation(self.cid, 10, 100000, "GENEREE")
...
class DashboardTests(TestCase):
    ...
    def test_dashboard_retourne_la_campagne_la_plus_recente(self) -> None:
        ...
        d = self.agg.get_dashboard()
        assert d.campagne is not None
        self.assertEqual(d.campagne.nom_campagne, "Juin")
        assert d.facturation is not None
        self.assertEqual(d.facturation.total_factures, 20)

    def test_stats_globales_agrege(self) -> None:
    ...
    def test_stats_campagne_inconnue_leve(self) -> None:
        with self.assertRaises(ObjectDoesNotExist):
            self.agg.get_stats_campagne(str(uuid.uuid4()))

    def test_stats_completes_agrege_les_3_domaines(self) -> None:
        cid = str(uuid.uuid4())
        ...
        assert d.facturation is not None
        self.assertEqual(d.facturation.total_factures, 20)
        assert d.paiements is not None
        self.assertEqual(d.paiements.montant_encaisse, Decimal("90000"))

    def test_stats_completes_campagne_inconnue_ne_leve_pas(self) -> None:
