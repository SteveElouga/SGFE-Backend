...
if not TESTING:
    ...
    _LOGGING_HANDLER_CONFIG["file"] = {
        ...
        # Chaînage de hash tamper-evident (voir stats/log_integrity.py,
...
LOGGING: dict[str, object] = {
    ...
    "filters": {
        "trace_context": {"()": "stats.logging_utils.TraceContextFilter"},
    },
    "formatters": {
        "iso8601_chained": {
            "()": "stats.log_integrity.ChainedHashFormatter",
            "format": "%(asctime)s.%(msecs)03dZ %(levelname)s %(name)s %(message)s",
            "datefmt": "%Y-%m-%dT%H:%M:%S",
        },
        # trace_id/span_id : voir stats/logging_utils.py::TraceContextFilter.
        "json": {
...
if env("PYROSCOPE_SERVER_ADDRESS", default=""):
    ...
    pyroscope.configure(
        ...
        tags={"service": "reporting-service"},
    )
