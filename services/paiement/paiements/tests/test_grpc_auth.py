...
def _details(metadata=(), methode="/paiement.PaiementService/GetSolde"):
    d = MagicMock()
    d.method = methode
...
class FailClosedTest(SimpleTestCase):
    ...
    def test_refuse_une_cle_vide(self):
        for valeur in ("", None):
            with self.assertRaises(CleInterneManquante):
                exiger_cle(valeur, "test")

    def test_le_serveur_refuse_de_demarrer_sans_cle(self):
    ...
    def test_le_client_refuse_de_partir_sans_cle(self):
        with self.assertRaises(CleInterneManquante):
            AuthClientInterceptor("")
...
class ServeurTest(SimpleTestCase):
    ...
    def test_laisse_passer_un_appel_correctement_signe(self):
    ...
    def test_refuse_un_appel_sans_metadonnee(self):
    ...
    def test_refuse_avant_d_atteindre_le_metier(self):
    ...
    def test_ne_journalise_pas_la_cle_recue(self):
...
class ClientTest(SimpleTestCase):
    def test_ajoute_la_cle_a_chaque_appel(self):
    ...
    def test_preserve_la_metadonnee_existante(self):
        interceptor = AuthClientInterceptor(CLE)
        suite = MagicMock(return_value="reponse")
    ...
    def test_preserve_le_delai_et_les_identifiants(self):
        interceptor = AuthClientInterceptor(CLE)
        suite = MagicMock(return_value="reponse")
...
class CanalTest(SimpleTestCase):
    def test_le_canal_est_bien_intercepte(self):
        ...
        self.assertNotIsInstance(canal, grpc.Channel.__class__)
