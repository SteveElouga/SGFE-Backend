...
class CampagneService:
    ...
    def demarrer_campagne(self, campagne_id: str) -> Campagne:
        ...
        if campagne.statut != StatutCampagne.PLANIFIEE:
            raise ValidationError(
                f"Seule une campagne PLANIFIEE peut être démarrée. Statut actuel : {campagne.statut}"
            )
    ...
    def cloturer_campagne(self, campagne_id: str) -> Campagne:
        ...
        if campagne.statut != StatutCampagne.EN_COURS:
            raise ValidationError(
                f"Seule une campagne EN_COURS peut être clôturée. Statut actuel : {campagne.statut}"
            )
    ...
    def list_campagnes(self, created_by: str = "") -> list[Campagne]:
        return self._repo.list_all(created_by=created_by)
    ...
    def ajouter_abonne_campagne(
        ...
        if campagne.statut not in (StatutCampagne.PLANIFIEE, StatutCampagne.EN_COURS):
            raise ValidationError(
                "Impossible d'ajouter un abonné à une campagne clôturée."
            )
        existant = self._releve_repo.get_by_campagne_abonne(campagne_id, abonne_id)
        if existant:
            raise ValidationError(
                f"L'abonné {abonne_id} est déjà inscrit à la campagne {campagne_id}."
            )
        return self._releve_repo.create(
            campagne=campagne, abonne_id=abonne_id, ancien_index=ancien_index
        )
    ...
    def demarrer_campagnes_planifiees_pour_aujourd_hui(self) -> list[Campagne]:
        ...
        for delta in (0, -1):
            ...
            if campagne:
                try:
                    updated = self._repo.update_statut(
                        campagne, StatutCampagne.EN_COURS
                    )
...
class ReleveService:
    ...
    def saisir_index(
        ...
        if releve.campagne.statut != StatutCampagne.EN_COURS:
            raise ValidationError(
                "Le relevé ne peut être saisi que sur une campagne EN_COURS."
            )
        ...
        if nouveau_index < releve.ancien_index:
            raise ValidationError(
                "Le nouvel index ne peut pas être inférieur à l'ancien index "
                f"({releve.ancien_index})."
            )
