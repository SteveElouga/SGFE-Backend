...
class AuditLog(models.Model):
    """Journal d'audit append-only des mutations du Campagne Service.

    Voir AUDIT_SGFE.md §10.7 (« Conception — propagation d'identité → journal
    d'audit immuable ») et §J (étape 2 — campagne, fusion de `ReleveAudit`).
    Une ligne par mutation métier, écrite par `campagnes.audit.enregistrer_audit`
    DANS LA MÊME transaction Django que le changement qu'elle documente —
    jamais un appel réseau séparé après coup.

    Coexiste avec `ReleveAudit` : celui-ci reste le journal métier d'un relevé
    précis, embarqué dans `ReleveResponse` pour l'affichage (historique
    saisie/correction d'UN relevé) ; `AuditLog` est le journal transverse du
    service (toutes les mutations, toutes les entités), pensé pour la preuve
    « qui a fait quoi » (CC7.2/CC7.3 SOC 2) et une future agrégation
    cross-service côté Reporting — pas pour remplacer `ReleveAudit`.
    ...
    - défense en profondeur, niveau base : la migration
      ...
      voir `campagnes/db_hardening.py` et AUDIT_SGFE.md §8·J).
    """

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    # Verbe métier de la mutation (ex. "CAMPAGNE_CREEE", "RELEVE_INDEX_SAISI").
    action = models.CharField(max_length=100)
    # Type de l'objet métier concerné (ex. "Campagne", "Releve").
    ...
    horodatage = models.DateTimeField(auto_now_add=True)
    # Détail libre, lisible par un humain (identifiants liés, index, motif...) —
    # pas de structure imposée : ce journal sert la preuve « qui a fait quoi
