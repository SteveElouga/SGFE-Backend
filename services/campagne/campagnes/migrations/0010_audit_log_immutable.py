...
L'immuabilité de premier niveau est déjà applicative : `campagnes.audit.
...
d'environnement `CAMPAGNE_DB_USER`, défaut `campagne_user` — voir
`campagne/settings.py`) — pas une chaîne codée en dur, pour rester correct
...
`0011_audit_log_role_runtime` (voir `campagnes/db_hardening.py` pour le
...
def _grant_update_delete(apps: ProjectState, schema_editor: BaseDatabaseSchemaEditor) -> None:
    """Réversible (`migrate campagnes 0009`) : rend UPDATE/DELETE au rôle applicatif."""
    role = _role_courant(schema_editor)
    if role is None:
        return
    with schema_editor.connection.cursor() as cursor:
