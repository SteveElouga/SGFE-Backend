"""Tests de la migration `0010_audit_log_immutable` (voir AUDIT_SGFE.md §10.7).
...
réels, vérifiés manuellement contre un Postgres jetable — voir la PR de
référence côté paiement) sans dépendre d'un vrai serveur Postgres dans cette
suite.
"""
...
_migration = importlib.import_module("notifications.migrations.0010_audit_log_immutable")
...
class RoleCourantTests(SimpleTestCase):
    ...
    def test_renvoie_le_role_configure_sur_postgresql(self) -> None:
        schema_editor = MagicMock()
        schema_editor.connection.vendor = "postgresql"
        schema_editor.connection.settings_dict = {"USER": "notification_user"}
        self.assertEqual(_migration._role_courant(schema_editor), "notification_user")
...
class RevokeGrantTests(SimpleTestCase):
    ...
    def test_revoke_execute_le_sql_attendu_sur_postgresql(self) -> None:
        ...
        cursor.execute.assert_called_once_with('REVOKE UPDATE, DELETE ON audit_log FROM "notification_user";')
    ...
    def test_grant_execute_le_sql_attendu_sur_postgresql(self) -> None:
        ...
        cursor.execute.assert_called_once_with('GRANT UPDATE, DELETE ON audit_log TO "notification_user";')
...
class MigrationOperationsTests(SimpleTestCase):
    ...
    def test_depend_de_la_migration_auditlog(self) -> None:
        self.assertIn(("notifications", "0009_auditlog"), _migration.Migration.dependencies)
