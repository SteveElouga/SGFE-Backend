...
le pouvait pas malgré `0010_audit_log_immutable` (voir AUDIT_SGFE.md §8·J et
`notifications/db_hardening.py` pour le constat empirique complet : le rôle
de connexion est un SUPERUTILISATEUR Postgres, qui contourne tout REVOKE tant
qu'aucun `SET ROLE` ne bascule la session sur un rôle non superutilisateur).

Gaté par `FORCE_POSTGRES_TESTS` (comme le job CI de la matrice `test`, qui
fait déjà tourner cette suite contre un `postgres:16-alpine` jetable — voir
...
propriétaire tout du long (migrations 0001-0011 appliquées avec les pleins
pouvoirs, comme en production). Le rôle `_runtime` créé par 0011 existe donc
...
from notifications.db_hardening import role_runtime
...
class RoleRuntimeBloqueReellementUpdateDeleteTests(TestCase):
    """Contre-exemple direct de ce que `0010_audit_log_immutable` documentait
    ...
    def test_le_role_proprietaire_peut_encore_modifier_avant_set_role(self) -> None:
        """Comportement historique (celui que documentait 0010) : sans
        `SET ROLE`, la connexion reste le rôle propriétaire/superutilisateur
        — la révocation de 0010 ne le bloque pas. Ce test fige ce fait pour
