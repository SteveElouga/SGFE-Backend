...
L'immuabilité de premier niveau est déjà applicative : `parametres.audit.
...
d'environnement `CONFIG_DB_USER`, défaut `config_user` — voir
`config/settings.py`) — pas une chaîne codée en dur, pour rester correct
si ce rôle est un jour renommé via l'environnement.

Note d'honnêteté — MISE À JOUR (voir `0005_audit_log_role_runtime`, qui
...
**L'investigation menée pour `0005` a montré que c'était en réalité plus
...
`0005_audit_log_role_runtime` (voir `parametres/db_hardening.py` pour le
...
habilité à faire du DDL). Cette migration-ci (`0004`) reste en place (le
...
c'est `0005`. Voir AUDIT_SGFE.md §8·J pour l'état à jour.
...
def _grant_update_delete(apps: ProjectState, schema_editor: BaseDatabaseSchemaEditor) -> None:
    """Réversible (`migrate parametres 0003`) : rend UPDATE/DELETE au rôle applicatif."""
    role = _role_courant(schema_editor)
    if role is None:
        return
    with schema_editor.connection.cursor() as cursor:
