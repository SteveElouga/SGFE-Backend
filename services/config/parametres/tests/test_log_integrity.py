...
`parametres/log_integrity.py` est une copie synchronisée de
...
(`config/settings.py`) pointe bien vers elle, sur le bon handler.
"""
...
from parametres.log_integrity import GENESIS_HASH, LOG_HASH_SUFFIX_RE, ChainedHashFormatter
...
class LoggingSettingsWiringTests(SimpleTestCase):
    ...
    def test_formatter_chaine_pointe_vers_parametres_log_integrity(self) -> None:
        ...
        self.assertEqual(formatter_chaine["()"], "parametres.log_integrity.ChainedHashFormatter")
    ...
    def test_handler_console_n_utilise_pas_le_formatter_chaine(self) -> None:
        ...
        # tant que `TESTING=True` (voir `config/settings.py` : ajouté seulement
