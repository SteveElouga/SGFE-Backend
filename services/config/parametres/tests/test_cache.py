...
class ConfigParamCacheTests(SimpleTestCase):
    def test_set_puis_get_retourne_la_valeur_avec_ttl_court(self):
        store: dict[str, str] = {}
        client = MagicMock()
    ...
    def test_get_sans_valeur_en_cache_retourne_none(self):
        client = MagicMock()
        client.get.return_value = None
        with patch.dict(sys.modules, {"redis": _fake_redis_module(client)}):
            self.assertIsNone(get_cached_param("cle_absente"))

    def test_invalidate_supprime_la_cle(self):
        client = MagicMock()
        with patch.dict(sys.modules, {"redis": _fake_redis_module(client)}):
    ...
    def test_invalidation_best_effort_sur_echec_redis(self):
        client = MagicMock()
        client.delete.side_effect = RuntimeError("redis down")
        with patch.dict(sys.modules, {"redis": _fake_redis_module(client)}):
            invalidate_param("delai_paiement_jours")  # ne doit pas lever

    def test_module_redis_absent_degrade_gracieusement(self):
...
class InfosSocieteCacheTests(SimpleTestCase):
    def test_set_puis_get_retourne_la_valeur(self):
        store: dict[str, str] = {}
        client = MagicMock()
        client.setex.side_effect = lambda key, ttl, value: store.__setitem__(key, value)
        client.get.side_effect = lambda key: store.get(key)
    ...
    def test_invalidate_supprime_la_cle_infos_societe(self):
        client = MagicMock()
        with patch.dict(sys.modules, {"redis": _fake_redis_module(client)}):
            invalidate_infos_societe()
