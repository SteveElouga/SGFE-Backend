...
le pouvait pas malgré `0004_audit_log_immutable` (voir AUDIT_SGFE.md §8·J et
`parametres/db_hardening.py` pour le constat empirique complet : le rôle de
...
Gaté par `FORCE_POSTGRES_TESTS` (comme le job CI `test-config`, qui fait
...
propriétaire tout du long (migrations 0001-0005 appliquées avec les pleins
pouvoirs, comme en production). Le rôle `_runtime` créé par 0005 existe donc
...
from parametres.db_hardening import role_runtime
...
class RoleRuntimeBloqueReellementUpdateDeleteTests(TestCase):
    """Contre-exemple direct de ce que `0004_audit_log_immutable` documentait
    ...
    def test_le_role_proprietaire_peut_encore_modifier_avant_set_role(self) -> None:
        """Comportement historique (celui que documentait 0004) : sans
        `SET ROLE`, la connexion reste le rôle propriétaire/superutilisateur
        — la révocation de 0004 ne le bloque pas. Ce test fige ce fait pour
