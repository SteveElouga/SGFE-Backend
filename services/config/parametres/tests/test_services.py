...
class ConfigServiceTests(TestCase):
    ...
    def test_get_known_key_returns_default_if_absent(self) -> None:
    ...
    def test_get_creates_record_in_db(self) -> None:
    ...
    def test_update_known_key_changes_value(self) -> None:
    ...
    def test_list_all_returns_all_defaults(self) -> None:
        params = self.service.list_all()
        cles = {p.cle for p in params}
        for cle in CONFIG_DEFAULTS:
            self.assertIn(cle, cles)

    def test_list_all_initializes_missing_defaults(self) -> None:
    ...
    def test_list_all_does_not_duplicate_existing(self) -> None:
        self.service.list_all()
        self.service.list_all()
        self.assertEqual(ConfigParam.objects.count(), len(CONFIG_DEFAULTS))

    def test_suspension_auto_default_is_true(self) -> None:
        param = self.service.get("impaye_suspension_auto")
        self.assertEqual(param.valeur, "true")

    def test_relance_etapes_defaults(self) -> None:
