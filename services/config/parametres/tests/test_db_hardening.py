"""Tests de `parametres/db_hardening.py` (voir AUDIT_SGFE.md §8·J).
...
from parametres import db_hardening as dh
...
class RoleProprietaireTests(SimpleTestCase):
    ...
    def test_renvoie_le_role_configure_sur_postgresql(self) -> None:
        connexion = MagicMock()
        connexion.vendor = "postgresql"
        connexion.settings_dict = {"USER": "config_user"}
        self.assertEqual(dh.role_proprietaire(connexion), "config_user")
...
class RoleRuntimeTests(SimpleTestCase):
    ...
    def test_suffixe_runtime_derive_du_role_proprietaire(self) -> None:
        ...
        connexion.vendor = "postgresql"
        connexion.settings_dict = {"USER": "config_user"}
        self.assertEqual(dh.role_runtime(connexion), "config_user_runtime")


class CreerRoleRuntimeTests(SimpleTestCase):
    def _schema_editor_postgres(self, role: str = "config_user") -> MagicMock:
    ...
    def test_cree_le_role_quand_absent_et_isole_la_table(self) -> None:
        ...
        cursor.execute.assert_any_call('CREATE ROLE "config_user_runtime" NOLOGIN;')
        cursor.execute.assert_any_call('GRANT "config_user_runtime" TO "config_user";')
        cursor.execute.assert_any_call('GRANT USAGE ON SCHEMA public TO "config_user_runtime";')
        cursor.execute.assert_any_call('GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO "config_user_runtime";')
        cursor.execute.assert_any_call(
            'GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO "config_user_runtime";'
        )
        cursor.execute.assert_any_call(
            'ALTER DEFAULT PRIVILEGES FOR ROLE "config_user" IN SCHEMA public '
            'GRANT ALL ON TABLES TO "config_user_runtime";'
        )
        cursor.execute.assert_any_call(
            'ALTER DEFAULT PRIVILEGES FOR ROLE "config_user" IN SCHEMA public '
            'GRANT ALL ON SEQUENCES TO "config_user_runtime";'
        )
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "audit_log" FROM "config_user_runtime";')
    ...
    def test_isole_plusieurs_tables(self) -> None:
        ...
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "audit_log" FROM "config_user_runtime";')
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "autre_table" FROM "config_user_runtime";')
...
class SupprimerRoleRuntimeTests(SimpleTestCase):
    ...
    def test_retire_les_droits_et_supprime_le_role_quand_present(self) -> None:
        ...
        cursor.execute.assert_any_call('GRANT UPDATE, DELETE ON "audit_log" TO "config_user_runtime";')
        cursor.execute.assert_any_call('REVOKE "config_user_runtime" FROM "config_user";')
        cursor.execute.assert_any_call('DROP OWNED BY "config_user_runtime";')
        cursor.execute.assert_any_call('DROP ROLE IF EXISTS "config_user_runtime";')
...
class DoitGarderRoleProprietaireTests(SimpleTestCase):
    ...
    def test_true_pour_test(self) -> None:
        with patch.object(sys, "argv", ["manage.py", "test", "parametres"]):
            self.assertTrue(dh._doit_garder_role_proprietaire())
...
class ActiverIsolementRuntimeTests(SimpleTestCase):
    ...
    def test_bascule_sur_le_role_runtime_pour_grpc_server(self) -> None:
        ...
        cursor.execute.assert_called_once_with('SET ROLE "config_user_runtime";')
