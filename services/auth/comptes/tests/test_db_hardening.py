"""Tests de `comptes/db_hardening.py` (voir AUDIT_SGFE.md §8·J).
...
from comptes import db_hardening as dh
...
class RoleProprietaireTests(SimpleTestCase):
    ...
    def test_renvoie_le_role_configure_sur_postgresql(self) -> None:
        connexion = MagicMock()
        connexion.vendor = "postgresql"
        connexion.settings_dict = {"USER": "auth_user"}
        self.assertEqual(dh.role_proprietaire(connexion), "auth_user")
...
class RoleRuntimeTests(SimpleTestCase):
    ...
    def test_suffixe_runtime_derive_du_role_proprietaire(self) -> None:
        ...
        connexion.vendor = "postgresql"
        connexion.settings_dict = {"USER": "auth_user"}
        self.assertEqual(dh.role_runtime(connexion), "auth_user_runtime")


class CreerRoleRuntimeTests(SimpleTestCase):
    def _schema_editor_postgres(self, role: str = "auth_user") -> MagicMock:
    ...
    def test_cree_le_role_quand_absent_et_isole_la_table(self) -> None:
        ...
        cursor.execute.assert_any_call('CREATE ROLE "auth_user_runtime" NOLOGIN;')
        cursor.execute.assert_any_call('GRANT "auth_user_runtime" TO "auth_user";')
        cursor.execute.assert_any_call('GRANT USAGE ON SCHEMA public TO "auth_user_runtime";')
        cursor.execute.assert_any_call('GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO "auth_user_runtime";')
        cursor.execute.assert_any_call('GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO "auth_user_runtime";')
        cursor.execute.assert_any_call(
            'ALTER DEFAULT PRIVILEGES FOR ROLE "auth_user" IN SCHEMA public GRANT ALL ON TABLES TO "auth_user_runtime";'
        )
        cursor.execute.assert_any_call(
            'ALTER DEFAULT PRIVILEGES FOR ROLE "auth_user" IN SCHEMA public '
            'GRANT ALL ON SEQUENCES TO "auth_user_runtime";'
        )
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "audit_log" FROM "auth_user_runtime";')
    ...
    def test_isole_plusieurs_tables(self) -> None:
        ...
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "audit_log" FROM "auth_user_runtime";')
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "autre_table" FROM "auth_user_runtime";')
...
class SupprimerRoleRuntimeTests(SimpleTestCase):
    ...
    def test_retire_les_droits_et_supprime_le_role_quand_present(self) -> None:
        ...
        cursor.execute.assert_any_call('GRANT UPDATE, DELETE ON "audit_log" TO "auth_user_runtime";')
        cursor.execute.assert_any_call('REVOKE "auth_user_runtime" FROM "auth_user";')
        cursor.execute.assert_any_call('DROP OWNED BY "auth_user_runtime";')
        cursor.execute.assert_any_call('DROP ROLE IF EXISTS "auth_user_runtime";')
...
class DoitGarderRoleProprietaireTests(SimpleTestCase):
    ...
    def test_true_pour_test(self) -> None:
        with patch.object(sys, "argv", ["manage.py", "test", "comptes"]):
            self.assertTrue(dh._doit_garder_role_proprietaire())
...
class ActiverIsolementRuntimeTests(SimpleTestCase):
    ...
    def test_bascule_sur_le_role_runtime_pour_grpc_server(self) -> None:
        ...
        cursor.execute.assert_called_once_with('SET ROLE "auth_user_runtime";')
