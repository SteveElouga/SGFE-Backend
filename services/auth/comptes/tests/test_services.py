...
class AuthServiceTests(TestCase):
    ...
    def test_login_by_phone_returns_tokens(self) -> None:
        access, refresh, _ = self.auth.login("+237690000001", "secret123")
        self.assertTrue(access)
        self.assertTrue(refresh)

    def test_login_unknown_username_raises(self) -> None:
    ...
    def test_login_with_unusable_password_raises(self) -> None:
        User.objects.create_user(
            username="pending1",
    ...
    def test_login_inactive_account_raises(self) -> None:
    ...
    def test_login_locks_after_max_attempts(self) -> None:
        for _ in range(5):
            with self.assertRaises(AuthenticationError):
                self.auth.login("comptable1", "wrongpassword")
    ...
    def test_login_messages_generiques_anti_enumeration(self) -> None:
    ...
    def test_validate_token_returns_user(self) -> None:
    ...
    def test_validate_token_invalid_raises(self) -> None:
        with self.assertRaises(AuthenticationError):
            self.auth.validate_token("token-invalide")

    def test_validate_token_deleted_user_raises(self) -> None:
        access, _, _ = self.auth.login("comptable1", "secret123")
        self.auth.users.get_by_username("comptable1").delete()
        with self.assertRaises(AuthenticationError):
            self.auth.validate_token(access)

    def test_validate_revoked_token_raises(self) -> None:
        access, _, _ = self.auth.login("comptable1", "secret123")
        self.auth.logout(access)
        with self.assertRaises(AuthenticationError):
            self.auth.validate_token(access)

    def test_refresh_token_returns_new_tokens(self) -> None:
    ...
    def test_refresh_token_revoque_lancien_refresh_token(self) -> None:
    ...
    def test_refresh_token_invalid_raises(self) -> None:
        with self.assertRaises(AuthenticationError):
            self.auth.refresh_token("token-invalide")

    def test_refresh_token_deleted_user_raises(self) -> None:
        _, refresh, _ = self.auth.login("comptable1", "secret123")
        self.auth.users.get_by_username("comptable1").delete()
        with self.assertRaises(AuthenticationError):
            self.auth.refresh_token(refresh)
    ...
    def test_refresh_revoked_token_raises(self) -> None:
        ...
        refresh_obj = RefreshTokenJWT(refresh)  # type: ignore[arg-type]
    ...
    def test_logout_invalid_token_raises(self) -> None:
        with self.assertRaises(AuthenticationError):
...
class UserAdminServiceTests(TestCase):
    ...
    def test_create_agent_has_no_usable_password(self) -> None:
    ...
    def test_create_agent_is_inactive_until_activation(self) -> None:
        user = self.user_admin.create_user(username="agent3z", phone_number="+237690000019", role=Role.AGENT)
        self.assertFalse(user.is_active)

    def test_create_admin_is_inactive_until_activation(self) -> None:
    ...
    def test_create_agent_creates_phone_otp_token(self) -> None:
    ...
    def test_create_admin_creates_password_setup_token(self) -> None:
        user = self.user_admin.create_user(
            username="admin3",
    ...
    def test_update_user_without_changes_keeps_values(self) -> None:
    ...
    def test_update_user_phone_change_resend_otp_si_non_active(self) -> None:
    ...
    def test_update_user_phone_change_ne_renvoie_pas_otp_si_actif(self) -> None:
    ...
    def test_update_user_admin_phone_change_ne_renvoie_pas_otp(self) -> None:
    ...
    def test_update_user_admin_email_change_resend_activation_si_en_attente(self) -> None:
    ...
    def test_update_user_admin_email_change_ne_renvoie_rien_si_actif(self) -> None:
    ...
    def test_deactivate_last_active_admin_raises(self) -> None:
        admin = User.objects.create_user(
            username="admin_last",
    ...
    def test_deactivate_admin_ok_when_another_active_admin_exists(self) -> None:
    ...
    def test_resend_credentials_non_admin_sends_otp(self) -> None:
    ...
    def test_resend_credentials_admin_pending_sends_activation_email(self) -> None:
        created = self.user_admin.create_user(
    ...
    def test_resend_credentials_admin_activated_sends_reset_email(self) -> None:
        created = self.user_admin.create_user(
...
class PasswordSetupServiceTests(TestCase):
    ...
    def test_send_activation_email_creates_valid_token(self) -> None:
        self.service.send_activation_email(self.user)
        token = PasswordSetupToken.objects.get(user=self.user)
        self.assertTrue(token.is_valid())
        self.mock_send.assert_called_once()

    def test_request_password_reset_admin_sends_email(self) -> None:
    ...
    def test_request_password_reset_non_admin_is_silent(self) -> None:
        User.objects.create_user(
    ...
    def test_request_password_reset_unknown_email_is_silent(self) -> None:
    ...
    def test_set_password_with_expired_token_raises(self) -> None:
        token = PasswordSetupToken.objects.create(user=self.user, expires_at=timezone.now() - timedelta(hours=1))
        with self.assertRaises(AuthenticationError):
            self.service.set_password_with_token(token.token, "newpassword123")

    def test_set_password_with_already_used_token_raises(self) -> None:
        token = PasswordSetupToken.objects.create(user=self.user, expires_at=timezone.now() + timedelta(hours=1))
        token.mark_used()
        with self.assertRaises(AuthenticationError):
            self.service.set_password_with_token(token.token, "newpassword123")

    def test_email_delivery_error_propagates(self) -> None:
...
class PhoneOtpServiceTests(TestCase):
    ...
    def test_send_otp_creates_token_and_sends_whatsapp(self) -> None:
    ...
    def test_send_otp_message_contient_lien_activation(self) -> None:
        self.service.send_otp(self.user)
    ...
    def test_request_otp_by_phone_sends_otp(self) -> None:
        self.service.request_otp_by_phone("+237690000040")
        self.mock_whatsapp.assert_called_once()

    def test_request_otp_unknown_phone_is_silent(self) -> None:
        self.service.request_otp_by_phone("+237600000000")
        self.mock_whatsapp.assert_not_called()

    def test_verify_otp_and_set_password_success(self) -> None:
        self.service.send_otp(self.user)
        import re

        match = re.search(r"\*(\d{6})\*", self.mock_whatsapp.call_args.kwargs["message"])
    ...
    def test_verify_otp_actives_pending_user(self) -> None:
    ...
    def test_request_otp_blocked_for_deactivated_user_with_password(self) -> None:
    ...
    def test_request_otp_allowed_for_pending_activation_user(self) -> None:
    ...
    def test_verify_wrong_otp_raises(self) -> None:
        self.service.send_otp(self.user)
        with self.assertRaises(AuthenticationError):
            self.service.verify_otp_and_set_password("+237690000040", "000000", "newpass")

    def test_previous_otps_invalidated_on_new_send(self) -> None:
        self.service.send_otp(self.user)
        self.service.send_otp(self.user)
    ...
    def test_verify_otp_compte_les_echecs(self) -> None:
    ...
    @override_settings(MAX_OTP_ATTEMPTS=3)
    def test_verify_otp_bloque_le_bruteforce_apres_plafond(self) -> None:
