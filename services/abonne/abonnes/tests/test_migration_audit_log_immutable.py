...
Réplique à l'identique les tests de la migration homonyme côté Paiement
Service (`0013_audit_log_immutable`).
...
_migration = importlib.import_module("abonnes.migrations.0010_audit_log_immutable")
...
class RoleCourantTests(SimpleTestCase):
    ...
    def test_renvoie_le_role_configure_sur_postgresql(self) -> None:
        schema_editor = MagicMock()
        schema_editor.connection.vendor = "postgresql"
        schema_editor.connection.settings_dict = {"USER": "abonne_user"}
        self.assertEqual(_migration._role_courant(schema_editor), "abonne_user")
...
class RevokeGrantTests(SimpleTestCase):
    ...
    def test_revoke_execute_le_sql_attendu_sur_postgresql(self) -> None:
        ...
        cursor.execute.assert_called_once_with('REVOKE UPDATE, DELETE ON audit_log FROM "abonne_user";')
    ...
    def test_grant_execute_le_sql_attendu_sur_postgresql(self) -> None:
        ...
        cursor.execute.assert_called_once_with('GRANT UPDATE, DELETE ON audit_log TO "abonne_user";')
...
class MigrationOperationsTests(SimpleTestCase):
    ...
    def test_depend_de_la_migration_auditlog(self) -> None:
        self.assertIn(("abonnes", "0009_auditlog"), _migration.Migration.dependencies)
