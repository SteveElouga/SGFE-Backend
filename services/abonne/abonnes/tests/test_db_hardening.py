"""Tests de `abonnes/db_hardening.py` (voir AUDIT_SGFE.md §8·J).
...
from abonnes import db_hardening as dh
...
class RoleProprietaireTests(SimpleTestCase):
    ...
    def test_renvoie_le_role_configure_sur_postgresql(self) -> None:
        connexion = MagicMock()
        connexion.vendor = "postgresql"
        connexion.settings_dict = {"USER": "abonne_user"}
        self.assertEqual(dh.role_proprietaire(connexion), "abonne_user")
...
class RoleRuntimeTests(SimpleTestCase):
    ...
    def test_suffixe_runtime_derive_du_role_proprietaire(self) -> None:
        ...
        connexion.vendor = "postgresql"
        connexion.settings_dict = {"USER": "abonne_user"}
        self.assertEqual(dh.role_runtime(connexion), "abonne_user_runtime")


class CreerRoleRuntimeTests(SimpleTestCase):
    def _schema_editor_postgres(self, role: str = "abonne_user") -> MagicMock:
    ...
    def test_cree_le_role_quand_absent_et_isole_la_table(self) -> None:
        ...
        cursor.execute.assert_any_call('CREATE ROLE "abonne_user_runtime" NOLOGIN;')
        cursor.execute.assert_any_call('GRANT "abonne_user_runtime" TO "abonne_user";')
        cursor.execute.assert_any_call('GRANT USAGE ON SCHEMA public TO "abonne_user_runtime";')
        cursor.execute.assert_any_call('GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO "abonne_user_runtime";')
        cursor.execute.assert_any_call(
            'GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO "abonne_user_runtime";'
        )
        cursor.execute.assert_any_call(
            'ALTER DEFAULT PRIVILEGES FOR ROLE "abonne_user" IN SCHEMA public '
            'GRANT ALL ON TABLES TO "abonne_user_runtime";'
        )
        cursor.execute.assert_any_call(
            'ALTER DEFAULT PRIVILEGES FOR ROLE "abonne_user" IN SCHEMA public '
            'GRANT ALL ON SEQUENCES TO "abonne_user_runtime";'
        )
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "audit_log" FROM "abonne_user_runtime";')
    ...
    def test_isole_plusieurs_tables(self) -> None:
        ...
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "audit_log" FROM "abonne_user_runtime";')
        cursor.execute.assert_any_call('REVOKE UPDATE, DELETE ON "autre_table" FROM "abonne_user_runtime";')
...
class SupprimerRoleRuntimeTests(SimpleTestCase):
    ...
    def test_retire_les_droits_et_supprime_le_role_quand_present(self) -> None:
        ...
        cursor.execute.assert_any_call('GRANT UPDATE, DELETE ON "audit_log" TO "abonne_user_runtime";')
        cursor.execute.assert_any_call('REVOKE "abonne_user_runtime" FROM "abonne_user";')
        cursor.execute.assert_any_call('DROP OWNED BY "abonne_user_runtime";')
        cursor.execute.assert_any_call('DROP ROLE IF EXISTS "abonne_user_runtime";')
...
class DoitGarderRoleProprietaireTests(SimpleTestCase):
    ...
    def test_true_pour_test(self) -> None:
        with patch.object(sys, "argv", ["manage.py", "test", "abonnes"]):
            self.assertTrue(dh._doit_garder_role_proprietaire())
...
class ActiverIsolementRuntimeTests(SimpleTestCase):
    ...
    def test_bascule_sur_le_role_runtime_pour_grpc_server(self) -> None:
        ...
        cursor.execute.assert_called_once_with('SET ROLE "abonne_user_runtime";')
