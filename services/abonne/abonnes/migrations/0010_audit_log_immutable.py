...
Réplique à l'identique la migration `0013_audit_log_immutable` du Paiement
Service (`services/paiement/paiements/migrations/`).

L'immuabilité de premier niveau est déjà applicative : `abonnes.audit.
...
d'environnement `ABONNE_DB_USER`, défaut `abonne_user` — voir
`abonne/settings.py`) — pas une chaîne codée en dur, pour rester correct si
ce rôle est un jour renommé via l'environnement.

Note d'honnêteté — MISE À JOUR (voir `0011_audit_log_role_runtime`, qui
...
**L'investigation menée pour `0011` a montré que c'était en réalité plus
...
`0011_audit_log_role_runtime` (voir `abonnes/db_hardening.py` pour le
...
habilité à faire du DDL). Cette migration-ci (`0010`) reste en place (le
...
c'est `0011`. Voir AUDIT_SGFE.md §8·J pour l'état à jour.
...
def _grant_update_delete(apps: ProjectState, schema_editor: BaseDatabaseSchemaEditor) -> None:
    """Réversible (`migrate abonnes 0009`) : rend UPDATE/DELETE au rôle applicatif."""
    role = _role_courant(schema_editor)
    if role is None:
        return
    with schema_editor.connection.cursor() as cursor:
