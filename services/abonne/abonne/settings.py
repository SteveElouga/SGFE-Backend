...
# positionnant FORCE_POSTGRES_TESTS=True (+ les ABONNE_DB_* habituels pointés
...
# superutilisateur (voir `abonnes/db_hardening.py` — copie synchronisée
...
from abonnes.db_hardening import connecter_isolement_runtime  # noqa: E402
...
if not TESTING:
    ...
    _LOGGING_HANDLER_CONFIG["file"] = {
        ...
        "filename": str(LOG_DIR / "abonne.log"),
        "when": "midnight",
        "utc": True,
        "backupCount": LOG_RETENTION_DAYS,
        # Chaînage de hash tamper-evident (voir abonnes/log_integrity.py,
...
LOGGING: dict[str, object] = {
    ...
    "filters": {
        "trace_context": {"()": "abonnes.logging_utils.TraceContextFilter"},
    },
    "formatters": {
        "iso8601_chained": {
            "()": "abonnes.log_integrity.ChainedHashFormatter",
            "format": "%(asctime)s.%(msecs)03dZ %(levelname)s %(name)s %(message)s",
            "datefmt": "%Y-%m-%dT%H:%M:%S",
        },
        # trace_id/span_id : voir abonnes/logging_utils.py::TraceContextFilter.
        "json": {
...
if env("PYROSCOPE_SERVER_ADDRESS", default=""):
    ...
    pyroscope.configure(
        application_name="abonne-service",
        server_address=env("PYROSCOPE_SERVER_ADDRESS", default=""),
        tags={"service": "abonne-service"},
    )
