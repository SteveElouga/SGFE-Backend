...
# positionnant FORCE_POSTGRES_TESTS=True (+ les FACTURATION_DB_* habituels
...
# superutilisateur (voir `factures/db_hardening.py` — copie synchronisée
...
from factures.db_hardening import connecter_isolement_runtime  # noqa: E402
...
if not TESTING:
    ...
    _LOGGING_HANDLER_CONFIG["file"] = {
        ...
        "filename": str(LOG_DIR / "facturation.log"),
        "when": "midnight",
        "utc": True,
        "backupCount": LOG_RETENTION_DAYS,
        # Chaînage de hash tamper-evident (voir factures/log_integrity.py,
...
LOGGING: dict[str, object] = {
    ...
    "filters": {
        "trace_context": {"()": "factures.logging_utils.TraceContextFilter"},
    },
    "formatters": {
        "iso8601_chained": {
            "()": "factures.log_integrity.ChainedHashFormatter",
            "format": "%(asctime)s.%(msecs)03dZ %(levelname)s %(name)s %(message)s",
            "datefmt": "%Y-%m-%dT%H:%M:%S",
        },
        # trace_id/span_id : voir factures/logging_utils.py::TraceContextFilter.
        "json": {
...
if env("PYROSCOPE_SERVER_ADDRESS", default=""):
    ...
    pyroscope.configure(
        ...
        tags={"service": "facturation-service"},
    )
