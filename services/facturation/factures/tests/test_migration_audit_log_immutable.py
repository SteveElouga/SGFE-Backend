"""Tests de la migration `0008_audit_log_immutable` (voir AUDIT_SGFE.md §10.7).
...
_migration = importlib.import_module("factures.migrations.0008_audit_log_immutable")
...
class RoleCourantTests(SimpleTestCase):
    ...
    def test_renvoie_le_role_configure_sur_postgresql(self) -> None:
        schema_editor = MagicMock()
        schema_editor.connection.vendor = "postgresql"
        schema_editor.connection.settings_dict = {"USER": "facturation_user"}
        self.assertEqual(_migration._role_courant(schema_editor), "facturation_user")
...
class RevokeGrantTests(SimpleTestCase):
    ...
    def test_revoke_execute_le_sql_attendu_sur_postgresql(self) -> None:
        ...
        cursor.execute.assert_called_once_with('REVOKE UPDATE, DELETE ON audit_log FROM "facturation_user";')
    ...
    def test_grant_execute_le_sql_attendu_sur_postgresql(self) -> None:
        ...
        cursor.execute.assert_called_once_with('GRANT UPDATE, DELETE ON audit_log TO "facturation_user";')
...
class MigrationOperationsTests(SimpleTestCase):
    ...
    def test_depend_de_la_migration_auditlog(self) -> None:
        self.assertIn(("factures", "0007_auditlog"), _migration.Migration.dependencies)
