...
le pouvait pas malgré `0008_audit_log_immutable` (voir AUDIT_SGFE.md §8·J et
`factures/db_hardening.py` pour le constat empirique complet : le rôle de
...
Gaté par `FORCE_POSTGRES_TESTS` (comme le job CI `test-facturation`, qui
...
propriétaire tout du long (migrations 0007-0010 appliquées avec les pleins
pouvoirs, comme en production). Le rôle `_runtime` créé par 0010 existe donc
...
from factures.db_hardening import role_runtime
from factures.models import AuditLog
...
class RoleRuntimeBloqueReellementUpdateDeleteTests(TestCase):
    """Contre-exemple direct de ce que `0008_audit_log_immutable` documentait
    ...
    def test_le_role_proprietaire_peut_encore_modifier_avant_set_role(self) -> None:
        """Comportement historique (celui que documentait 0008) : sans
        `SET ROLE`, la connexion reste le rôle propriétaire/superutilisateur
        — la révocation de 0008 ne le bloque pas. Ce test fige ce fait pour
