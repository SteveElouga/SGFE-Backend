...
def _facture(svc: "FactureService", **kw: Any) -> Facture:
    defauts = dict(
        abonne_id=ABONNE,
...
class AnnulerFactureTest(TestCase):
    ...
    def test_la_facture_reste_au_journal_avec_son_numero(self) -> None:
        f = _facture(self.svc)
    ...
    def test_le_motif_et_l_auteur_sont_conserves(self) -> None:
        f = _facture(self.svc)
    ...
    def test_le_motif_vide_est_refuse(self) -> None:
        f = _facture(self.svc)
        for motif in ("", "   "):
    ...
    def test_le_solde_est_eteint_avant_le_marquage(self) -> None:
        ...
        self.svc._paiement_client.annuler_solde.assert_called_once()  # type: ignore[attr-defined]
        appel = self.svc._paiement_client.annuler_solde.call_args.kwargs  # type: ignore[attr-defined]
        self.assertEqual(appel["facture_id"], str(f.id))

    def test_un_service_paiement_indisponible_empeche_l_annulation(self) -> None:
        f = _facture(self.svc)
        self.svc._paiement_client.annuler_solde.side_effect = RuntimeError("paiement KO")  # type: ignore[attr-defined]
        with self.assertRaises(RuntimeError):
            self.svc.annuler_facture(str(f.id), motif="Erreur", annule_par="admin")
        f.refresh_from_db()
        self.assertEqual(f.statut, StatutFacture.IMPAYEE)

    def test_annuler_deux_fois_est_refuse(self) -> None:
        f = _facture(self.svc)
        self.svc.annuler_facture(str(f.id), motif="Erreur", annule_par="admin")
        with self.assertRaises(ValidationError):
            self.svc.annuler_facture(str(f.id), motif="Encore", annule_par="admin")

    def test_une_regularisation_s_annule_aussi(self) -> None:
    ...
    def test_annulation_retire_la_facture_impayee_des_stats(self, mock_pub: MagicMock) -> None:
    ...
    @patch("factures.services.publish_reporting_event")
    def test_annulation_d_une_facture_deja_payee_le_signale(self, mock_pub: MagicMock) -> None:
...
class RegenererFactureTest(TestCase):
    ...
    def _relevé(self, ancien: str = "100", nouveau: str = "130") -> None:
    ...
    def test_la_nouvelle_facture_suit_le_releve_corrige_pas_l_ancienne(self) -> None:
    ...
    def test_les_deux_factures_se_citent(self) -> None:
        f = _facture(self.svc)
        self._relevé()
        annulee, nouvelle = self.svc.regenerer_facture(
    ...
    def test_la_nouvelle_porte_un_numero_different(self) -> None:
        f = _facture(self.svc)
        self._relevé()
        annulee, nouvelle = self.svc.regenerer_facture(
            str(f.id), motif="Correction", regenere_par="admin", delai_paiement_jours=15
    ...
    def test_un_solde_est_ouvert_pour_la_nouvelle(self) -> None:
    ...
    def test_une_regularisation_ne_se_regenere_pas(self) -> None:
    ...
    def test_sans_releve_la_regeneration_est_refusee(self) -> None:
        f = _facture(self.svc)
        self.svc._campagne_client.list_releves.return_value = []  # type: ignore[attr-defined]
    ...
    def test_un_releve_incoherent_est_refuse_avant_d_annuler(self) -> None:
        f = _facture(self.svc)
    ...
    def test_campagne_service_indisponible_n_annule_rien(self) -> None:
        f = _facture(self.svc)
        self.svc._campagne_client.list_releves.side_effect = RuntimeError("campagne KO")  # type: ignore[attr-defined]
        with self.assertRaises(PreconditionError):
            self.svc.regenerer_facture(str(f.id), motif="Correction", regenere_par="admin", delai_paiement_jours=15)
        f.refresh_from_db()
        self.assertEqual(f.statut, StatutFacture.IMPAYEE)
