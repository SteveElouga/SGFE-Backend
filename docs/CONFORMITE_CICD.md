...
| 12 | Signature/provenance générées dans le même job que le build (backend) — SLSA Build L3 non atteint | SLSA Build L3 | **L** | Migrer vers `slsa-framework/slsa-github-generator` pour isoler la génération de provenance du build applicatif. **✅ Fait ET vérifié en conditions réelles — PR #209 (backend, 05/09/2026).** Job `provenance` isolé (`generator_container_slsa3.yml`), `cosign verify-attestation` réancré sur l'identité OIDC du générateur. Au premier `push` réel sur `develop` après fusion, les 11 jobs `publish-*` et leur job de provenance imbriqué ont tous réussi sans exception. |
