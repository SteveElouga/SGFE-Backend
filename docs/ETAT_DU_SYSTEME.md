...
**Tests** : 73 tests, tous verts (confirmé par exécution ; +12 sur PR #41 pour le gabarit HTML et le client Abonné, +13 sur PR #52 pour le versioning/régénération PDF, la commande `regenerer_pdfs` et le délai de règlement dynamique). Le rendu PDF réel (WeasyPrint) est vérifié séparément par un smoke-test dans l'image Docker (`%PDF-`), car il dépend de bibliothèques natives absentes de l'environnement de CI. Les tests de service/gRPC déclenchent de vrais appels réseau vers Paiement/Notification/Abonné (non mockés) qui échouent proprement en local (`ConnectionRefused` capté par la dégradation gracieuse) — fonctionne mais n'est pas une pratique de test unitaire isolée idéale.
