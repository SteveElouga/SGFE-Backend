...
**ANO-030 — whatsapp-service : la session WhatsApp est perdue à chaque redémarrage du container (impossible d'envoyer les factures PDF)** — ✅ **RÉSOLU** (branche `fix/whatsapp-session-persistante-remoteauth`, PR à venir)

- **Constat** : `whatsapp-service` utilisait `LocalAuth`, qui persiste la session dans le profil Chromium sur le volume `whatsapp_session`. Le volume était pourtant bien monté et la lib à jour (`whatsapp-web.js@1.34.7`), mais **`server.js` n'avait aucun handler d'arrêt** : sur `docker stop`, Node était tué sans prévenir Puppeteer, qui tuait alors Chromium par SIGKILL. Les clés de session multi-device de WhatsApp Web (IndexedDB/LevelDB, écrites de façon différée) n'étaient jamais flushées sur disque → au redémarrage, profil incohérent → réémission du QR → session perdue. Symptôme : `renvoyerFactureWhatsapp` renvoyait `statut: ECHEC`, `erreur: "WhatsApp non connecté — scannez le QR code sur /qr"` après chaque redémarrage.
- **Correctif appliqué** : (1) **arrêt gracieux** — handlers `SIGTERM`/`SIGINT` dans `server.js` qui appellent `client.destroy()` (fermeture propre du navigateur) avant de sortir, `stop_grace_period: 30s` côté compose. (2) **Persistance externalisée** — bascule de `LocalAuth` vers `RemoteAuth` avec un `Store` custom (`whatsapp-service/redis-store.js`) adossé à **Redis** (déjà dans la stack) : la session est zippée et sauvegardée hors du container (au boot + toutes les 5 min), puis restaurée au démarrage. La session survit désormais au redémarrage, au rebuild d'image et à une perte du volume `whatsapp_session`. (3) **Redis rendu persistant** — le volume `redis_data` (déclaré mais jamais monté) est maintenant monté sur `/data` avec AOF activé (`--appendonly yes`), pour que la session survive aussi à un redémarrage de Redis lui-même. **Limite connue** : la toute première sauvegarde a lieu ≈ 60 s après le premier scan du QR (délai imposé par RemoteAuth) — un redémarrage dans cette fenêtre initiale exigerait un nouveau scan ; le log `Session sauvegardée dans Redis` confirme que la session est sécurisée.
...
- **ANO-032** — ✅ **RÉSOLU** (branche `fix/facturation-date-releve-datetime`, PR à venir) — **Le flux réel de génération de factures était cassé** : `SaisirIndex` (Campagne) horodate `date_releve` (`DateTimeField`), donc `ReleveResponse.date_releve` arrive en **datetime ISO** (`2026-07-04T17:10:12+00:00`), mais Facturation faisait `datetime.date.fromisoformat(releve.date_releve)` (`services.py:143`, commentée `# ISO date "YYYY-MM-DD"`) → `ValueError: Invalid isoformat string` → `GenererFactures` échoue en `INTERNAL`. **Impossible de générer une facture depuis un vrai relevé** ; la seule facture existante venait d'un seed (date propre), ce qui masquait le bug. Découvert en créant un impayé cohérent de test via le vrai flux (campagne → saisie index → clôture → génération). Corrigé : parsing robuste (essaie `date.fromisoformat`, retombe sur `datetime.fromisoformat(...).date()`), on ne conserve que la date pour la date limite. Test de non-régression ajouté (`date_releve` en datetime → 22 tests verts). Le time du relevé reste porté par Campagne (utile pour l'heure de relevé du PDF, ANO-027).
...
- **25 queries**, **35 mutations**, **subscriptions temps réel** (Redis pub/sub) : `abonneUpdated` (ADMIN, ANO-015), `whatsappStatus` (ADMIN, PR #60 — statut/QR WhatsApp poussé sans polling), `factureUpdated(campagneId)` + `paiementCree(campagneId)` (ADMIN/COMPTABLE) qui poussent factures et paiements en temps réel, et `utilisateurUpdated(utilisateurId)`. Canaux dédiés : `facture:events` (facturation-service : `GenererFactures`/`UpdateStatutFacture`), `paiement:events` (paiement-service : `EnregistrerPaiement`, événement **auto-porteur** faute de `GetPaiement`) et `user:events` (auth-service : `CreateUser`/`UpdateUser`/`DeactivateUser`/`ReactivateUser`). Enfin `configUpdated(cle)` (ADMIN, canal `config:events` : `UpdateConfig`), `tarifUpdated` (ADMIN/COMPTABLE, canal `tarif:events` : `UpdateTarif`) et `progressionUpdated(campagneId)` (canal `progression:events` : `SaisirIndex`). Permettent au front de passer en cache-first et de supprimer les refetch `network-only`. Côté requêtes, `whatsappQr` (ADMIN, PR #46) relaie le QR de liaison sans exposer la clé interne du whatsapp-service.
- Contrôle de rôle systématique via `require_role(info, *roles)` sur toutes les opérations sensibles, y compris les subscriptions : chacune reprend le rôle de la query équivalente (`abonneUpdated`/`whatsappStatus`/`configUpdated` → ADMIN ; `factureUpdated`/`paiementCree`/`tarifUpdated` → ADMIN/COMPTABLE). Cas particulier `utilisateurUpdated` : ADMIN pour le flux global, mais un utilisateur non-ADMIN peut suivre **son propre id** (cas « profil » : réagir en direct à un changement de son rôle / sa désactivation — sécurité). `progressionUpdated` : ADMIN/AGENT/SUPERVISEUR, mais un SUPERVISEUR/AGENT ne voit que ses propres campagnes (via `_verifier_acces_campagne`) et le flux global est réservé à l'ADMIN. **Corrigé le 3 septembre 2026 :** `infosSociete` était documentée comme exception volontaire (publique, au motif qu'elle « alimente les PDF »). Vérifié que ce motif ne correspond à aucun besoin réel — le PDF de facture est rendu par `facturation-service`, qui appelle `config-service` en gRPC directement, jamais via cette query GraphQL — et que le seul consommateur frontend est l'écran `/configuration`, déjà derrière `roleGuard(['ADMIN'])`. `require_auth(info)` y a été ajouté ; il n'y a donc plus d'exception.
...
**whatsapp-service (Node.js)** : `whatsapp-web.js` + Puppeteer/Chromium headless, session persistée dans Redis via `RemoteAuth` (zip du profil sauvegardé périodiquement + arrêt gracieux — ANO-030 résolu, remplace l'ancien `LocalAuth` sur volume qui perdait la session à chaque redémarrage), reconnexion avec backoff exponentiel sur déconnexion. Endpoints `GET /health`, `GET /qr` (page HTML du QR), `GET /qr-data` (QR en JSON `{ready, qr}` pour relais UI admin, PR #46), `POST /send`, `POST /send-with-pdf`. Authentification par clé partagée (en-tête `X-Internal-Api-Key`) sur tous sauf `/health` (ANO-005 résolu, PR #21).

**Liaison WhatsApp depuis l'UI admin (PR #46)** : le QR de connexion, autrefois seulement atteignable sur l'endpoint interne `/qr` (401 dans un navigateur faute d'en-tête), est désormais exposé à l'admin via la chaîne gRPC `Gateway (query ADMIN whatsappQr) → notification-service (GetWhatsAppQr) → whatsapp-service (/qr-data)`. La clé interne reste côté serveur. **Push temps réel** : whatsapp-service publie tout changement d'état (nouveau QR, connecté, déconnecté) sur le canal Redis `whatsapp:events` ; la subscription GraphQL `whatsappStatus` (ADMIN, `gateway/schema/subscriptions.py`) le pousse au navigateur via WebSocket, avec un snapshot initial — l'UI n'a plus à poller (la query `whatsappQr` reste comme repli / fetch ponctuel). Dégradation gracieuse si whatsapp-service est indisponible (`ready=False, qr=""`). `ARCHITECTURE.md` §5.7 affirmait à tort un « Retry Handler — 3 tentatives » : **il n'y a en réalité aucune logique de retry**, ni côté Node ni côté Django (échec immédiatement marqué `ECHEC`, dégradation gracieuse sans nouvelle tentative) — documentation corrigée (ANO-009 résolu, PR #25).
