...
| Config Service          | 🟡 Fonctionnel mais **silencieusement inefficace** | 29 ✅                    | **Bug actif** : incohérence de casse des clés (ANO-001)                            |
...
| ------------------------------------------------------------------------ | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `consommation = nouveau_index - ancien_index` (toujours ≥ 0)             | 🟡 Partiel                      | Validé côté **Campagne** (`campagnes/services.py:130`). **Non revalidé côté Facturation** (ANO-008) — confiance aveugle dans les données reçues.                                                                                                                                                                                                         |
...
| `date_limite = date_releve + delai_paiement_jours` (défaut 5)            | 🔴 **Cassé en pratique**        | Le calcul est correct, mais `delai_paiement_jours` n'est **jamais lu avec succès** depuis Config Service (ANO-001) — la valeur utilisée est toujours le défaut codé en dur de `facturation/settings.py`, jamais celle qu'un ADMIN configurerait.                                                                                                         |
| `date_expiration_token = date_envoi + token_validite_jours` (défaut 20)  | 🔴 **Cassé en pratique**        | Même cause (ANO-001) — `token_validite_jours` n'est jamais lu avec succès non plus.                                                                                                                                                                                                                                                                      |
...
| Statut ACTIF de l'abonné vérifié avant ajout en campagne                 | 🔴 **Non implémenté**           | Ni Abonné ni Campagne ne font cette vérification (ANO-003) — un abonné `SUSPENDU`/`RESILIE` peut être relevé et facturé normalement.                                                                                                                                                                                                                     |
...
**ANO-001 — Config Service : incohérence de casse des clés rend le paramétrage ADMIN sans effet**
...
- **Correctif** : uniformiser la casse (recommandé : minuscule partout, aligné sur le style Python/gRPC des autres services) et ajouter les 6 clés `impaye_`* manquantes côté Config Service, ou faire correspondre les noms à `RELANCE_ETAPE_1..4_JOURS` déjà existants s'ils sont sémantiquement équivalents (à trancher : la sémantique « étape de relance » et « délai avant rappel/avertissement/suspension » n'est pas garantie identique).
...
- **Correctif** : appeler `AbonneServiceClient.get_abonne(abonne_id).statut == "ACTIF"` avant la création du `Releve` (dans `SaisirIndex` ou en amont), ou refuser silencieusement en `NON_RELEVE` avec motif.

**ANO-003bis — L'événement** `AbonneCreated` **(Abonné → Campagne) documenté dans CLAUDE.md n'existe pas**

- **Réalité** : Abonné Service publie `ABONNE_CREATED`/`ABONNE_UPDATED` uniquement sur **Redis pub/sub** (`services/abonne/abonnes/event_publisher.py:6-25`, canal `abonne:events`), consommé **uniquement par la Gateway** pour les subscriptions GraphQL temps réel. Campagne Service ne s'y abonne jamais et n'a pas de client Abonné fonctionnel (voir ANO-003). L'intégration event-driven décrite dans CLAUDE.md racine (§ « Événements inter-services ») n'existe donc pas pour ce flux précis.
