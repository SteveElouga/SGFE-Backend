...
- **25 queries**, **35 mutations**, **subscriptions temps réel** (Redis pub/sub) : `abonneUpdated` (ADMIN, ANO-015), `whatsappStatus` (ADMIN, PR #60 — statut/QR WhatsApp poussé sans polling), et désormais `factureUpdated(campagneId)` + `paiementCree(campagneId)` (ADMIN/COMPTABLE) qui poussent factures et paiements en temps réel. Canaux dédiés `facture:events` (émis par facturation-service : `GenererFactures`/`UpdateStatutFacture`) et `paiement:events` (émis par paiement-service : `EnregistrerPaiement`, événement **auto-porteur** faute de `GetPaiement`) — permettent au front de passer en cache-first et de supprimer les refetch `network-only`. Côté requêtes, `whatsappQr` (ADMIN, PR #46) relaie le QR de liaison sans exposer la clé interne du whatsapp-service.
- Contrôle de rôle systématique via `require_role(info, *roles)` sur toutes les opérations sensibles, y compris les subscriptions : chacune reprend le rôle de la query équivalente (`abonneUpdated`/`whatsappStatus` → ADMIN ; `factureUpdated`/`paiementCree` → ADMIN/COMPTABLE). Seule exception volontaire : `infosSociete` (public — sert à alimenter les PDF).
